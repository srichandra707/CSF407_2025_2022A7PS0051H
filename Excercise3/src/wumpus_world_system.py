...
class WumpusBN:
    def __init__(self,n):
        ...
        self.inference=None
        self.evidence={}
        self._create_network_structure()
            ...
            for j in range(self.n):
                if (i,j) not in visited and self.is_reachable(current_pos, (i, j), max_depth):
                    ...
                    if not np.isnan(pit_values).any():
                        ...
                        pit_prob = 0.8  # Mark as high risk if probability is undefined
...
class EnhancedWumpusWorld(wumpusworld):
    ...
    def visualize_risk(self):
        risk_map = self.bn.get_pit_risk(self.agent.current_pos, self.agent.visited)

        plt.figure(figsize=(8, 6))
        plt.imshow(risk_map, cmap='Reds', vmin=0, vmax=1)
        plt.colorbar(label='Pit Probability')
        plt.scatter(self.agent.current_pos[1], self.agent.current_pos[0],c='blue', s=200, edgecolors='white')
